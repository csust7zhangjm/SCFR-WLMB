...
##########################################################################
# Spatial Attention Layer (RGC-SFEB)
...
class SALayer(nn.Module):
    ...
    def reconstruct(self, x_1, x_2):
        ...
        v1 = torch.cat([x_11 + x_22, x_12 + x_21], dim=1)
        v2 = torch.cat([x_11 + x_12, x_22 + x_21], dim=1)
        v = 0.5 * v1 + 0.5 * v2
        return v
...
class HWB(nn.Module):
    ...
    def forward(self, x):
        ...
        x_dwt = self.dwt1(wavelet_path_in)
        res = self.body(x_dwt)
        ...
        out_channel = int(res.shape[1] / 4)
        LL = res[:, 0:out_channel, :, :] / 2
        LH = res[:, out_channel:out_channel * 2, :, :] / 2
        HL = res[:, out_channel * 2:out_channel * 3, :, :] / 2
        HH = res[:, out_channel * 3:out_channel * 4, :, :] / 2
        wavelet_path = self.iwt1(LL, HL, LH, HH)
...
##########################################################################
##---------- SCFR_WLMB-LOL ----------
class SCFR_WLMB(nn.Module):
    def __init__(self, in_chn=3, wf=64, depth=4):
        super(SCFR_WLMB, self).__init__()
        self.depth = depth
        ...
        for i in range(depth):  # 0,1,2,3
            ...
            if i < 2:
                self.down_path.append(UNetConvBlock(wf, (2 ** i) * wf, downsample))
            else:
                self.down_path.append(UNetConvBlock(wf * (2**(i-1)), wf * (2 ** i), downsample))
                final_channels = wf * (2 ** i)

        # decoder of UNet-64
        self.up_path = nn.ModuleList()
        self.skip_conv = nn.ModuleList()
        self.conv_up = nn.ModuleList()
        self.bottom_conv = nn.Conv2d(final_channels, wf, 3, 1, 1)
        self.bottom_up = bili_resize(2 ** (depth-1))

        prev_channels = 0
        for i in reversed(range(depth - 1)):
            self.up_path.append(UNetUpBlock((2 ** (i+1)) * wf, (2 ** i) * wf))
...
if __name__ == "__main__":
    ...
    model = SCFR_WLMB(in_chn=3, wf=96, depth=4).cuda()
    out = model(input)
